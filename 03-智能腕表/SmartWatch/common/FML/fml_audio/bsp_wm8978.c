...
static uint8_t wm8978_i2cWrite(uint8_t addr, uint16_t dat)
{
    I2C_Start();
    I2C_WriteByte(WM8978_SLAVE_ADDRESS);
    if (I2C_WaitAck())
    {
        I2C_Stop();
        return 1;
    }
    I2C_WriteByte( ((addr<<1)&0xFE) | ((dat>>8)&0x01) );
    if (I2C_WaitAck())
    {
        I2C_Stop();
        return 2;
    }
    I2C_WriteByte(dat&0xFF);
    if (I2C_WaitAck())
    {
        I2C_Stop();
        return 3;
...
{
	uint8_t res;
	res=wm8978_i2cWrite(_ucRegAddr,_usValue);
  if(!res)
    wm8978_RegCash[_ucRegAddr] = _usValue;
	return res;
}

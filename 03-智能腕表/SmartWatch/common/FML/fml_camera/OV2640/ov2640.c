...
{ 
  ...
  RCC_AHB1PeriphClockCmd(OV2640_PWDN_RCC, ENABLE);
  GPIO_InitStructure.GPIO_Pin = OV2640_PWDN_PIN;//IO
  GPIO_Init(OV2640_PWDN_GPIO, &GPIO_InitStructure);//初始化
  
  RCC_AHB1PeriphClockCmd(OV2640_RST_RCC, ENABLE);
  GPIO_InitStructure.GPIO_Pin = OV2640_RST_PIN;//IO
  GPIO_Init(OV2640_RST_GPIO, &GPIO_InitStructure);//初始化
  ...
  //初始化 OV2640分辨率
  OV2640_ResolutionConfig();
  
  return 0x00; 	//ok
} 

void OV2640_ResolutionConfig()
{
  u16 i = 0;

...
#define LIS3DH_CS_L()   do {GPIO_ResetBits(GPIOB, GPIO_Pin_12);} while(0)
#define LIS3DH_CS_H()   do {GPIO_SetBits(GPIOB, GPIO_Pin_12);}while(0)
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t* Data);


void  LIS3DH_Reg_Dump(void);
...
void LIS3DH_init(void){ 
  ...
  LIS3DH_ReadReg(LIS3DH_WHO_AM_I,&id);

  if (id == 0x33) {
    //printf("********** lis3dh before init ********\r\n");
    //LIS3DH_Reg_Dump();
    ...
    //printf("haha : find ic lis3dh\r\n");
    //printf("*********** lis3dh after init *************\r\n");
    //LIS3DH_Reg_Dump();
  } else printf("error can't find ic lis3dh\r\n");
}
/*******************************************************************************
...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t* Data) {
  ...
  LIS3DH_CS_L();
  //delay_ms(1);
    /**Data = */SPI2_ReadWriteByte(Reg|0x80);

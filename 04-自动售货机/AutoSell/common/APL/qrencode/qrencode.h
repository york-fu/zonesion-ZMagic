 ...
 */
typedef enum {
	QR_MODE_NUL = -1,  ///< Terminator (NUL character). Internal use only终结者（NUL字符）.内部使用
	QR_MODE_NUM = 0,   ///< Numeric mode数字模式
	QR_MODE_AN,        ///< Alphabet-numeric mode字母数字模式
	QR_MODE_8,         ///< 8-bit data mode8位数据模式
	QR_MODE_KANJI,     ///< Kanji (shift-jis) mode日本汉字(shift-jis)模式
	QR_MODE_STRUCTURE, ///< Internal use only内部使用
	QR_MODE_ECI,       ///< ECI mode出口信用保险模式
	QR_MODE_FNC1FIRST,  ///< FNC1, first position第一位置
	QR_MODE_FNC1SECOND, ///< FNC1, second position第二位置
} QRencodeMode;

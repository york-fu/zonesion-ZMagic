...
int package_data_send(char *zxbee)
{
  ...
  process_post_synch(&u_zhiyun,LteLedEvent,NULL);
...
static void package_decode(char *buf, int len)
{
  ...
  else if (strstr(buf, "method") && strstr(buf, "control"))
  {
...
{
  static struct etimer et_zhiyun; 

  PROCESS_BEGIN();

  LteLedEvent = process_alloc_event();
  tcp_con = tcp_alloc();
  lteConfig_init();
  ...
  while (1)
  {
    ...
    if (ev == evt_tcp)
    {
      ...
      else if (ptcp->status == TCP_STATUS_CLOSED)
      {
        ...
        etimer_set(&timer, CLOCK_SECOND); //重连延时
      }
    }
    ...
    if (ev == PROCESS_EVENT_TIMER && etimer_expired(&timer))
    {
      ...
      else
      {
        if (tcp_con->status == TCP_STATUS_CLOSED || tcp_con->status == TCP_STATUS_OPEN)
        {
          ...
          etimer_set(&timer, CLOCK_SECOND*5); //超时重连
        }
        if (tcp_con->status == TCP_STATUS_CONNECTED)
        {

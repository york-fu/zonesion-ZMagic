...
const char Spo2_TitleText[][32] = {
  "Blood oxygen curve",
  "血氧饱和度曲线",
};

const char Spo2_Text1[][32] = {
  "Current value:",
  "当前测量值：",
};

static GRAPH_DATA_Handle hData;
static short DataBuffer[SPO2_BUFFER_SIZE] = {0};
// USER END
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    TEXT_SetText(hItem, Spo2_TitleText[System.language]);
    //
    ...
    hData = GRAPH_DATA_YT_Create(GUI_RED, SPO2_BUFFER_SIZE, DataBuffer, 0);
    ...
    TEXT_SetFont(hItem, FontList16[System.font]);
    sprintf(buf,"%s 000",Spo2_Text1[System.language]);
  ...
  case WM_TIMER:
    ...
    hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_1);
    sprintf(buf,"%s %d",Spo2_Text1[System.language],spo2);
    TEXT_SetText(hItem, buf);
...
*       CreateSPO2
*/
WM_HWIN CreateSPO2(void);
WM_HWIN CreateSPO2(void) {
  WM_HWIN hWin;

  hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
  WM_CreateTimer(WM_GetClientWindow(hWin), 0x06, 200, 0); //创建一个软件定时器
  return hWin;
}

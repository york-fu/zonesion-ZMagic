...
int m_nSymbleSize;
m_byModuleData_t* m_byModule = NULL;
...
void FormatModule(void)
{
	...
	memset(m_byModule->Data, 0, sizeof(m_byModule->Data));
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		...
		{
			m_byModule->Data[i][j] = (BYTE)((m_byModule->Data[i][j] & 0x11) != 0);
		}
	}
}
//设置功能性模块信息
...
void SetFunctionModule(void)
{
	...
	for (i = 0; i < 8; ++i)
	{
		m_byModule->Data[i][7] = m_byModule->Data[7][i] = '\x20';
		m_byModule->Data[m_nSymbleSize - 8][i] = m_byModule->Data[m_nSymbleSize - 8 + i][7] = '\x20';
		m_byModule->Data[i][m_nSymbleSize - 8] = m_byModule->Data[7][m_nSymbleSize - 8 + i] = '\x20';
	}

	for (i = 0; i < 9; ++i)
	{
		m_byModule->Data[i][8] = m_byModule->Data[8][i] = '\x20';
	}

	for (i = 0; i < 8; ++i)
	{
		m_byModule->Data[m_nSymbleSize - 8 + i][8] = m_byModule->Data[8][m_nSymbleSize - 8 + i] = '\x20';
	}
	...
	{
		m_byModule->Data[i][6] = (i % 2) == 0 ? '\x30' : '\x20';
		m_byModule->Data[6][i] = (i % 2) == 0 ? '\x30' : '\x20';
	}
}
//设置大回字信息
// O X X X X X X X
...
void SetFinderPattern(int x, int y)
{
	...
	for (i = 0; i < 7; ++i)
	{
		for (j = 0; j < 7; ++j)
		{
			m_byModule->Data[x + j][y + i] = (byPattern[i] & (1 << (6 - j))) ? '\x30' : '\x20'; 
		}
	}
}
//设置小回字信息
...
void SetAlignmentPattern(int x, int y)
{
	...
	for (i = 0; i < 5; ++i)
	{
		for (j = 0; j < 5; ++j)
		{
			m_byModule->Data[x + j][y + i] = (byPattern[i] & (1 << (4 - j))) ? '\x30' : '\x20'; 
		}
	}
}
...
void SetVersionPattern(void)
{
	...
	for (i = 0; i < 6; ++i)
	{
		for (j = 0; j < 3; ++j)
		{
			m_byModule->Data[m_nSymbleSize - 11 + j][i] = m_byModule->Data[i][m_nSymbleSize - 11 + j] =
...
void SetCodeWordPattern(void)
{
	...
	for (i = 0; i < m_ncAllCodeWord; ++i)
	{
		for (j = 0; j < 8; ++j)
		{
			...
			while (m_byModule->Data[x][y] & 0x20); 

			m_byModule->Data[x][y] = (m_byAllCodeWord[i] & (1 << (7 - j))) ? '\x02' : '\x00';
		}
	}
}
//设置遮蔽信息
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize; ++j)
		{
			if (! (m_byModule->Data[j][i] & 0x20)) 
			{
				...
				m_byModule->Data[j][i] = (BYTE)((m_byModule->Data[j][i] & 0xfe) | (((m_byModule->Data[j][i] & 0x02) > 1) ^ bMask));
			}
		}
	}
}
...
void SetFormatInfoPattern(int nPatternNo)
{
	...
	for (i = 0; i <= 5; ++i)
		m_byModule->Data[8][i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	m_byModule->Data[8][7] = (nFormatData & (1 << 6)) ? '\x30' : '\x20';
	m_byModule->Data[8][8] = (nFormatData & (1 << 7)) ? '\x30' : '\x20';
	m_byModule->Data[7][8] = (nFormatData & (1 << 8)) ? '\x30' : '\x20';

	for (i = 9; i <= 14; ++i)
		m_byModule->Data[14 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	for (i = 0; i <= 7; ++i)
		m_byModule->Data[m_nSymbleSize - 1 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	m_byModule->Data[8][m_nSymbleSize - 8] = '\x30';

	for (i = 8; i <= 14; ++i)
		m_byModule->Data[8][m_nSymbleSize - 15 + i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';
}
...
int CountPenalty(void)
{
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 4; ++j)
		{
			...
			{
				if (((m_byModule->Data[i][j] & 0x11) == 0) == ((m_byModule->Data[i][k] & 0x11) == 0))
					++nCount;
				else
					break;
			}
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 4; ++j)
		{
			...
			for (k = j + 1; k < m_nSymbleSize; k++)
			{
				if (((m_byModule->Data[j][i] & 0x11) == 0) == ((m_byModule->Data[k][i] & 0x11) == 0))
					++nCount;
				else
					break;
			}
	...
	for (i = 0; i < m_nSymbleSize - 1; ++i)
	{
		...
		{
			if ((((m_byModule->Data[i][j] & 0x11) == 0) == ((m_byModule->Data[i + 1][j]		& 0x11) == 0)) &&
				(((m_byModule->Data[i][j] & 0x11) == 0) == ((m_byModule->Data[i]	[j + 1] & 0x11) == 0)) &&
				(((m_byModule->Data[i][j] & 0x11) == 0) == ((m_byModule->Data[i + 1][j + 1] & 0x11) == 0)))
			{
				nPenalty += 3;
			}
		}
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		...
		{
			if (((j == 0) ||				 (! (m_byModule->Data[i][j - 1] & 0x11))) && // 柧 傑偨偼 僔儞儃儖奜
											 (   m_byModule->Data[i][j]     & 0x11)   && // 埫 - 1
											 (! (m_byModule->Data[i][j + 1] & 0x11))  && // 柧 - 1
											 (   m_byModule->Data[i][j + 2] & 0x11)   && // 埫 劉
											 (   m_byModule->Data[i][j + 3] & 0x11)   && // 埫 劆3
											 (   m_byModule->Data[i][j + 4] & 0x11)   && // 埫 劊
											 (! (m_byModule->Data[i][j + 5] & 0x11))  && // 柧 - 1
											 (   m_byModule->Data[i][j + 6] & 0x11)   && // 埫 - 1
				((j == m_nSymbleSize - 7) || (! (m_byModule->Data[i][j + 7] & 0x11))))   // 柧 傑偨偼 僔儞儃儖奜
			{
				if (((j < 2 || ! (m_byModule->Data[i][j - 2] & 0x11)) && 
					 (j < 3 || ! (m_byModule->Data[i][j - 3] & 0x11)) &&
					 (j < 4 || ! (m_byModule->Data[i][j - 4] & 0x11))) ||
					((j >= m_nSymbleSize - 8  || ! (m_byModule->Data[i][j + 8]  & 0x11)) &&
					 (j >= m_nSymbleSize - 9  || ! (m_byModule->Data[i][j + 9]  & 0x11)) &&
					 (j >= m_nSymbleSize - 10 || ! (m_byModule->Data[i][j + 10] & 0x11))))
				{
					nPenalty += 40;
				}
			}
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (m_byModule->Data[j - 1][i] & 0x11))) && // 柧 傑偨偼 僔儞儃儖奜
											 (   m_byModule->Data[j]    [i] & 0x11)   && // 埫 - 1
											 (! (m_byModule->Data[j + 1][i] & 0x11))  && // 柧 - 1
											 (   m_byModule->Data[j + 2][i] & 0x11)   && // 埫 劉
											 (   m_byModule->Data[j + 3][i] & 0x11)   && // 埫 劆3
											 (   m_byModule->Data[j + 4][i] & 0x11)   && // 埫 劊
											 (! (m_byModule->Data[j + 5][i] & 0x11))  && // 柧 - 1
											 (   m_byModule->Data[j + 6][i] & 0x11)   && // 埫 - 1
				((j == m_nSymbleSize - 7) || (! (m_byModule->Data[j + 7][i] & 0x11))))   // 柧 傑偨偼 僔儞儃儖奜
			{
				if (((j < 2 || ! (m_byModule->Data[j - 2][i] & 0x11)) && 
					 (j < 3 || ! (m_byModule->Data[j - 3][i] & 0x11)) &&
					 (j < 4 || ! (m_byModule->Data[j - 4][i] & 0x11))) ||
					((j >= m_nSymbleSize - 8  || ! (m_byModule->Data[j + 8][i]  & 0x11)) &&
					 (j >= m_nSymbleSize - 9  || ! (m_byModule->Data[j + 9][i]  & 0x11)) &&
					 (j >= m_nSymbleSize - 10 || ! (m_byModule->Data[j + 10][i] & 0x11))))
				{
					nPenalty += 40;
				}
			}
...
//--	SendCmd(3, 0x1b, 0x4a, 25);
	...
	{	
		...
		{	
			k = i*bit_v;			
			if(m_byModule->Data[j][k] == 1 && m_byModule->Data[j][k+1] == 1)
			{
				print_buff[x] = 0xFF;
				x++;
			}
			else if(m_byModule->Data[j][k] == 1 && m_byModule->Data[j][k+1] == 0)
			{
				print_buff[x] = 0xF0;
				x++;
			}
			else if(m_byModule->Data[j][k] == 0 && m_byModule->Data[j][k+1] == 1)
			{
				print_buff[x] = 0x0F;
				x++;
			}
